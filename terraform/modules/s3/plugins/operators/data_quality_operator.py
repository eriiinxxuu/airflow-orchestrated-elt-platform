...
─────────────────────────
Runs SQL quality checks against Redshift staging tables.
Raises an exception if any check fails.

Design:
  - checks is a list of dicts, each defining one check
  - Each check contains a sql query (returning a single scalar) and an expected value
  - expected can be a callable to support range checks (e.g. lambda n: n > 0)
  - Factory functions (row_count_check, etc.) generate common checks
    to reduce boilerplate in DAG files
"""
...
# ── Factory functions — generate common check dicts ───────────────────────────
...
def duplicate_check(table: str, keys: list[str]) -> dict:
    """Check that the given key columns have no duplicate combinations."""
    key_expr = ", ".join(keys)
    return {
